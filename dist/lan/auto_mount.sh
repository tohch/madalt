...
BASE_MOUNT_ROOT="/mnt/smb"  # Корневая точка для autofs
...
generate_map_filename() {
    local server_clean="${SERVER//[^a-zA-Z0-9._-]/_}"
    echo "/etc/auto.d/auto.smb_${server_clean}"
}
...
add_to_auto_master() {
    ...
    # Формат: /mnt/smb/server_name -fstype=autofs,--ghost,--timeout=60 /etc/auto.d/auto.smb_server
...
EOF
    ...
    local autofs_opts="${SMB}noauto,x-systemd.automount,x-systemd.idle-timeout=60,_netdev,rw,credentials=$CRED_FILE,soft,file_mode=0777,dir_mode=0777,nofail"

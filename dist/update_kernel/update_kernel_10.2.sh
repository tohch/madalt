...
#===============================================================================
# Проверка прав доступа и окружения (требуется запуск через su -)
#===============================================================================
check_root() {
    local flags=""
    [[ "$AUTO_YES" == "true" ]] && flags="-y"
    
    # 1. Проверка наличия прав root (UID 0)
    if [ "$(id -u)" -ne 0 ]; then
        echo -e "${YELLOW}[!] Требуются права root. Введите пароль:${NC}"
        # Перезапуск через su - с login-shell
        # Устанавливаем маркер __ALT_LOGIN_SHELL=1, чтобы не зацикливаться
        exec su - root -c "__ALT_LOGIN_SHELL=1 AUTO_YES='$AUTO_YES' bash -l \"$(realpath "$0")\" $flags"
    fi
    
    # 2. UID == 0, но нужно убедиться, что мы в правильном login-shell окружении
    # Если маркер не установлен — значит, скрипт запущен НЕ через наш перезапуск
    if [[ "$__ALT_LOGIN_SHELL" != "1" ]]; then
        warn "Скрипт запущен не из под пользователя!"
        warn "Выполняется автоматический перезапуск через 'su -' с login-shell..."
        
        # Перезапуск с установкой маркера
        exec su - root -c "__ALT_LOGIN_SHELL=1 AUTO_YES='$AUTO_YES' bash -l \"$(realpath "$0")\" $flags"
    fi
    
    # Если дошли сюда — всё ок: root + правильное окружение
    success "Скрипт запущен с правами root в login-shell окружении"
}

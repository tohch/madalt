    ...
    while [[ $retry -lt $max_retries_upgrade ]]; do
        ...
        if [[ $upgrade_status -eq 0 ]]; then
            ...
            if [[ $retry -lt $max_retries_upgrade ]]; then
                ...
                error "Вариант 1: при загрузке выберите загрузку с дополнительными параметров, далее выберете предыдущее ядро."
                error "Вариант 2: если система вообще не загружается, то загрузиться с LiveUSB, выполнить chroot и запустить: apt-get -f install && dpkg --configure -a"
